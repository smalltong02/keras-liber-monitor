...
int _tmain(int argc, _TCHAR* argv[])
{
    ...
    if (g_hook_test_object->Initialize() && g_log_object->Initialize(g_impl_object->GetLogMode()))
    {
        //Sleep(1000*100);
...
int _tmain(int argc, _TCHAR* argv[])
{
    if (g_log_object->Initialize(g_impl_object->GetLogMode()) && g_hook_test_object->Initialize())
    {
