...
#define DEL_POST_PROCESSING(NAME, FUNC) DelPostProcessing(#NAME, reinterpret_cast<CFunction::func_proto>(FUNC))
	...
	CHookImplementObject() :
		m_bValid(false), m_benable(false), m_configObject(nullptr) { CoInitializeEx(0, COINIT_MULTITHREADED); }//for hook wmi methods
	~CHookImplementObject() {
...
private:
	...
	sf::safe_ptr<std::map<std::string, std::vector<int>>> m_delayNodeList;
};
