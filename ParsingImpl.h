...
public:
	...
	CPrototype() = delete;
	CPrototype(_call_convention conv, const std::string& name) : m_call_conv(conv), m_proto_name(name), m_special(false) {}
	CPrototype(_call_convention conv, const std::string& library, const std::string& name, const PrototypeArguments& args, bool special = false) : m_call_conv(conv), m_library(library), m_special(special), m_proto_name(name), m_args(args) {}
	~CPrototype() = default;
