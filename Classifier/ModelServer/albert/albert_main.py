...
import sys
import argparse
from albertmodel import InnerAlbertInstance

def albert_main(args):
    if args.input is None or args.input == "":
        print("Input path is missing. Please provide --input with a valid path.")
        sys.exit(0)
    elif len(args.input) and (os.path.exists(args.input) == False or os.path.isfile(args.input) == False):
    ...
    if args.predict == True and (args.model_path is None or args.model_path == ""):
        ...
        sys.exit(0)
    if args.test == True and (args.model_path is None or args.model_path == ""):
        print("model path must be specified when testing.")
        sys.exit(0)
    if args.test == True and args.predict == True:
        print("testing and predicting are in conflict.")
        sys.exit(0)
    if args.test == False and args.predict == False and (args.output is None or args.output == ""):
    ...
    if args.output is not None and (os.path.exists(args.output) == False or os.path.isdir(args.output) == False):
        ...
        sys.exit(0)
    if args.model_path is not None and (os.path.exists(args.model_path) == False or os.path.isfile(args.model_path) == False):
