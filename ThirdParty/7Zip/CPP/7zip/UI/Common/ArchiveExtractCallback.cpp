...
HRESULT CArchiveExtractCallback::GetExtractStream(CMyComPtr<ISequentialOutStream> &outStreamLoc, bool &btest, bool &needExit)
{
  needExit = true;
  ...
  if(_bmemmode)
    _outFileStreamSpec->SetMode(COutFileStream::out_mode::mode_memory);
  ...
  else // not reprase
  {
    if (_ntOptions.PreAllocateOutFile && !_isSplit && _curSizeDefined && _curSize > (1 << 12))
    {
      ...
      bool res = _outFileStreamSpec->SetLength(_curSize);
...
STDMETHODIMP CArchiveExtractCallback::GetStream(UInt32 index, ISequentialOutStream **outStream, Int32 askExtractMode)
{
  ...
  *outStream = NULL;
  ...
  if (askExtractMode == NArchive::NExtract::NAskMode::kExtract || askExtractMode == NArchive::NExtract::NAskMode::kTest)
  {
    if (_stdOutMode)
    ...
    {
        bool needExit = true;
        bool btest = askExtractMode == NArchive::NExtract::NAskMode::kTest ? true : false;
        RINOK(GetExtractStream(outStreamLoc, btest, needExit));
        if (needExit)
            return S_OK;
    }
  }
...
HRESULT CArchiveExtractCallback::CloseFile()
{
  ...
  {
    bool res = _outFileStreamSpec->SetLength(processedSize);
    _fileLengthWasSet = res;
    if (!res)
    {
...
STDMETHODIMP CArchiveExtractCallback::SetOperationResult(Int32 opRes)
{
  ...
  if (_callback) {
      if (_outFileStreamSpec) {
          _callback(GetPackagePath(), _item, _outFileStreamSpec->GetData());
      }
      else {
          _callback(GetPackagePath(), _item, {});
      }
  }
  if (!_bmemmode) {
...
STDMETHODIMP CArchiveExtractCallback::CryptoGetTextPassword(BSTR* password)
{
    COM_TRY_BEGIN
        if (DefaultPwd.IsEmpty()) {
            ...
            if (password) {
                *password = SysAllocString(DefaultPwd);
            }
            return S_OK;
        }
  COM_TRY_END
