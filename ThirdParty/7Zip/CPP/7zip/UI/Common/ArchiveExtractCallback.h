...
public:
  ...
  typedef bool (*c7zcallback)(const FString& path, CReadArcItem&, const std::vector<unsigned char>&);
  ...
  void Set7zCallback(c7zcallback callback) {
      _callback = callback;
      return;
  }
  void SetDefaultPassword(const FString& pwd) {
      DefaultPwd = pwd;
  }
...
private:
  ...
  HRESULT GetExtractStream(CMyComPtr<ISequentialOutStream> &outStreamLoc, bool& bmemorymode, bool &needExit);
