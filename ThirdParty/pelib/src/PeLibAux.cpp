...
namespace PeLib
{
  ...
  // Weird syntax to make Borland C++ happy
	...
	**/
	...
	{
		PeFile32 pef(stream, PeLib::PeFileT<32>::loading_memory);
		return getFileType(pef);
	}
	...
	**/
	...
	PeFile* openPeFile(std::istream& stream)
	{
		...
		if (type == PEFILE32)
		{
			return new PeFile32(stream, PeLib::PeFileT<32>::loading_memory);
		}
		else if (type == PEFILE64)
		{
			return new PeFile64(stream, PeLib::PeFileT<64>::loading_memory);
		}
		else
		{
			return nullptr;
