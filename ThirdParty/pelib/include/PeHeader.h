...
#define IMAGE_NT_SIGNATURE                  0x00004550  // PE00
		  ...
		  PeHeaderT() : m_uiOffset(0), m_checksumFileOffset(0), m_secDirFileOffset(0), m_ldrError(LDR_ERROR_NONE), expansion_mode(false)
		  {
		  }
          void SetExpansionMode(bool mode);
...
// image directory getters
    ...
    template<int x>
    void PeHeaderT<x>::SetExpansionMode(bool mode)
    { 
        expansion_mode = mode; 
    }
...
//		obBuffer << m_inthHeader.OptionalHeader.BaseOfData;
	...
	**/
	...
	typename FieldSizes<x>::VAR4_8 PeHeaderT<x>::rvaToOffset(VAR4_8 dwRva) const
	{
        ...
        if (expansion_mode) return dwRva;
