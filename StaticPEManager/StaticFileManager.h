...
namespace cchips {
    ...
    public:
        ...
        bool Scan(const std::string& scan_path, const std::string& output_path, bool boutput_console = false) {
...
#define _mongodb_document_limit 16777216
        ...
        bool scan_callback(const std::string& scan_path, const std::string& output_path) {
            ...
            case CJsonOptions::_config::_outputtype::output_mongodbformat:
            {
                if (GenerateMongodbFormat(std::move(fileinfo))) {
                    return true;
                }
            }
            break;
        ...
        bool GenerateMongodbFormat(std::unique_ptr<CFileInfo> fileinfo) {
            try {
                mongocxx::client client{mongocxx::uri{_mongodb_default_uri}};
